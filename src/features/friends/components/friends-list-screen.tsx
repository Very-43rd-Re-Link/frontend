...
type FriendsListScreenProps = {
    friendsCount: number;
    recommendedFriends: FriendOverview[];
    friends: FriendListItem[];
};

export function FriendsListScreen({ friendsCount, recommendedFriends, friends }: FriendsListScreenProps) {
    return (
        <main className="relative flex min-h-0 flex-1 flex-col gap-5 bg-relink-white px-5 pt-10">
            ...
            <FriendSearchBar />
            <FriendList friends={friends} sortLabel="기본" referenceTime="20:30" />
        </main>
    );
}
